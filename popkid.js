...
setTimeout(() => {
    ...
    async function main() {
        ...
        const sockOptions = {
           ...
           return msg?.message; // Returns `undefined` if message not found
           }
          };
    ...
    try {
        ...
        try {
            ...
            return isIdentityQuestion 
                ? `I'm DULLAH-XMD, created by Ibrahim Adams! 🚀\n\n${textResponse}`
                : textResponse;
        }
    } catch (error) {
...
if (conf.CHATBOT === "yes" || conf.DULLAH_CHATBOT === "yes") {
...
zk.ev.on("messages.upsert", async (m) => {
    try {
        ...
        const { trackActivity } = require("./lib/trackActivity"); // <- required here

        const from = msg.key.remoteJid;
        const isGroup = from.endsWith("@g.us");
        const sender = msg.key.participant || msg.key.remoteJid;

        // ===== 📊 ACTIVITY TRACKING START =====
        try {
            if (
                from &&
                from.endsWith("@g.us") &&
                ...
                await trackActivity(msg);
            }
        } catch (err) {
            console.error("❌ Error in activity tracking:", err.message);
        }
        // ===== 📊 ACTIVITY TRACKING END =====

        // ===== 🛡️ ANTIBUG START =====
        // Read antibug settings from ./bdd/antibug.json
        const antibugPath = path.join(__dirname, "./bdd/antibug.json");
        let antibugSettings = { status: "off" };
        if (fs.existsSync(antibugPath)) {
            try {
                antibugSettings = JSON.parse(fs.readFileSync(antibugPath));
            } catch (e) {
                console.error("❌ Failed to parse antibug.json:", e);
            }
        }

        if (antibugSettings.status === "on") {
            if (
                ...
                (msg.message.documentMessage && msg.message.documentMessage.mimetype === "application/octet-stream")
            ) {
                await zk.sendMessage(from, { delete: msg.key });
                await zk.sendMessage(
                    from,
                    {
                        text: `⚠️ Bug detected and removed by *DULLAH XMD Antibug Shield*`,
                        mentions: [sender],
                    },
                    { quoted: msg }
                );
                console.log("🛡️ Antibug deleted a suspicious message in:", from);
        ...
        // ===== 🧹 ANTI-STICKER START =====
        const antistickerPath = path.join(__dirname, "./data/antisticker.json");
        const antistickerData = fs.existsSync(antistickerPath)
            ? JSON.parse(fs.readFileSync(antistickerPath))
            : { groups: {} };
        ...
        if (
            isGroup &&
            isSticker &&
            antistickerData.groups[from] &&
            antistickerData.groups[from].active
        ) {
            await zk.sendMessage(from, { delete: msg.key });
            console.log("🧹 Sticker deleted in group:", from);

            await zk.sendMessage(from, {
                text: "✅ Sticker has been deleted by *DULLAH XMD*.",
                mentions: [msg.key.participant]
            }, { quoted: msg });
        }
        // ===== 🧹 ANTI-STICKER END =====

        // ===== 🔞 ANTIBAD (GLOBAL VIA SET.JS) =====
const s = require("./set");

const badWords = [
  // English
  "fuck", "fucking", "bitch", "shit", "asshole", "dick", "nigga", "whore", "motherfucker", "faggot", "cock", "slut", "cunt",
  // Swahili
  "mpumbavu", "tumbavu", "kuma", "bozi", "mavi", "kilaza", "kichwa maji", "mchafu", "malaya", "mjinga",
  "jambazi", "mwizi", "mshenzi", "chupi", "hujuwi kuoga", "we kuma", "fala",
  // Urdu
  "madarchod", "behenchod", "gandu", "lund", "chutiya", "randi", "bhosdike",
  // Ghanaian
  "kwasia", "gyimii", "aboa", "jon", "tweaa", "abonsam", "kobokobo", "you dey craze",
  // Zimbabwean
  "bhurugwa", "mukadzi wechikomba", "chibharo", "garwe", "ngwena", "chimbwido", "zvinyadzi", "mukaka", "tsvuku",
  "mombe", "mbudzi", "gudo", "mbavha", "mwana wehure", "kapfupi", "tete", "chikomba", "dzvinyu", "tsotsi", "husiku hwehusiku"
];

zk.ev.on('messages.upsert', async (m) => {
  try {
    if (s.ANTIBAD.toLowerCase() !== "yes") return;
    ...
    for (let word of badWords) {
      if (messageText.includes(word)) {
        ...
        await zk.sendMessage(
          from,
          {
            text: `⚠️ Message deleted due to use of bad language.`,
            mentions: [sender],
          },
          { quoted: msg }
        );
    ...
    console.error('Error in global antibad handler:', e);
  }
});
// ===== 🔞 ANTIBAD END =====
        ...
        // ===== 🚨 ANTI-TAG-ADMIN START =====
        const { getStatus } = require("./bdd/antitagadmin");

        const isActive = await getStatus(from);
        if (isGroup && isActive) {
            ...
            if (mentions.length > 0) {
                ...
                const mentionedAdmin = mentions.find(jid => admins.includes(jid));
                const isSenderAdmin = admins.includes(sender);

                if (mentionedAdmin && !isSenderAdmin) {
                    await zk.sendMessage(from, { delete: msg.key });

                    await zk.sendMessage(from, {
                        text: `🚫 *Warning:* Do not mention admins in this group!`,
                        mentions: [sender]
                    }, { quoted: msg });

                    console.log("🚨 AntiTagAdmin: Deleted due to admin mention in", from);
                }
            }
        }
        // ===== 🚨 ANTI-TAG-ADMIN END =====
        ...
        // ===== 🚫 ANTI-TAG SPAM START =====
        const autoTagPath = path.join(__dirname, './bdd/autotag.json');
        const tagWarnPath = path.join(__dirname, './bdd/tagwarns.json');

        const autoTagDB = fs.existsSync(autoTagPath)
            ? JSON.parse(fs.readFileSync(autoTagPath))
            : {};

        const warnDB = fs.existsSync(tagWarnPath)
            ? JSON.parse(fs.readFileSync(tagWarnPath))
            : {};
        ...
        if (isGroup && autoTagDB[from] === 'on') {
            const lowerBody = text?.toLowerCase() || '';
            const tagTriggers = ['.tag', '.tagall', '.hidetag', '@all', '.everyone', '.all'];

            const metadata = await zk.groupMetadata(from);
            const admins = metadata.participants.filter(p => p.admin).map(p => p.id);
            const isSenderAdmin = admins.includes(sender);

            const isTagSpam = tagTriggers.some(cmd => lowerBody.includes(cmd));
            if (isTagSpam && !isSenderAdmin) {
                // Count warning
                warnDB[from] = warnDB[from] || {};
                warnDB[from][sender] = (warnDB[from][sender] || 0) + 1;
                fs.writeFileSync(tagWarnPath, JSON.stringify(warnDB, null, 2));

                const warnCount = warnDB[from][sender];

                await zk.sendMessage(from, {
                    text: `⚠️ *${msg.pushName}*, please do not tag everyone.\nWarning: *${warnCount}/3*`,
                    mentions: [sender]
                }, { quoted: msg });

                await zk.sendMessage(from, { react: { text: '⚠️', key: msg.key } });
                await zk.sendMessage(from, { delete: msg.key });

                if (warnCount >= 3) {
                    await zk.sendMessage(from, {
                        text: `❌ *${msg.pushName}* has been removed for repeated mass tagging.`,
                        mentions: [sender]
                    });
        ...
        // ===== 🤖 AUTOSTICKER USING media/autosticker.json START =====
        if (Config.AUTO_STICKER === "yes" && text) {
            const jsonPath = path.join(__dirname, "media", "autosticker.json");

            if (fs.existsSync(jsonPath)) {
                const stickerMap = JSON.parse(fs.readFileSync(jsonPath));
                const userMessage = text.toLowerCase();

                for (let keyword in stickerMap) {
                    if (userMessage.includes(keyword)) {
                        const stickerFile = stickerMap[keyword];
                        const stickerPath = path.join(__dirname, "media", stickerFile);

                        if (fs.existsSync(stickerPath)) {
                            await zk.sendMessage(from, { sticker: { url: stickerPath } }, { quoted: msg });
                        } else {
                            console.warn(`⚠️ Sticker file not found: ${stickerPath}`);
                        }
                ...
                console.warn("⚠️ autosticker.json file not found in media/");
            }
        }
        // ===== 🤖 AUTOSTICKER END =====
